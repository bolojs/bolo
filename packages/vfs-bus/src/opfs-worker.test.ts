...
function createMockWorker(responses: Record<string, any>): Worker {
  ...
  return {
    ...
    set onmessage(fn: ((e: any) => void) | null) { onmessageFn = fn; },
...
describe('OpfsWorker + VfsBus cold layer', () => {
  it('readFile falls back to cold and promotes to hot', async () => {
    ...
    expect(new Uint8Array(hotData as unknown as ArrayBuffer)).toEqual(new Uint8Array([99, 111, 108, 100]));
  ...
  it('writeFile writes to hot and cold', async () => {
    ...
    mockW.postMessage = (msg: any) => { coldWrites.push(`${msg.method}:${msg.path}`); origPost(msg); };
