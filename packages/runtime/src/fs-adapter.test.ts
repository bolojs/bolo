...
describe('createFileSystem', () => {
  ...
  it('should readdir with withFileTypes returning DirEnt array', async () => {
    ...
    const fileEnt = (entries as Array<{ name: string; isFile(): boolean; isDirectory(): boolean }>).find((e) => e.name === 'file.txt');
    const dirEnt = (entries as Array<{ name: string; isFile(): boolean; isDirectory(): boolean }>).find((e) => e.name === 'folder');
