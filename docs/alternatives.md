...
description: How bolo compares to Node.js, WebContainers, and AlmostNode.
---
...
|                        | **bolo**                  | **Node.js**       | **WebContainers**  | **AlmostNode**    |
| ---------------------- | -------------------------- | ------------------ | ------------------- | ------------------ |
| **Where it runs**       | Browser                    | Server / CLI        | Browser              | Browser             |
| **License**             | Apache 2.0                 | MIT                 | Proprietary          | MIT                 |
| **npm published**       | Yes                        | n/a                 | Yes                  | Yes                 |
| **Boot time**            | ~100ms (QuickJS) / ~500ms (V8 worker) | Instant (native) | 2 to 5s          | Instant             |
| **Node.js compat**       | Partial (shims)             | Full (it is Node)  | Full (via WASM)      | Partial (40+ shims) |
| **Native packages (NAPI)** | No                       | Yes                 | Yes                  | No                  |
| **VFS + persistence**    | Yes (memfs + OPFS)          | Native filesystem   | Yes                  | Yes                 |
| **AI agent sandbox**     | Yes (QuickJS, C-level caps) | No                  | No                   | Cross-origin sandbox only |
| **`boot()`-style API**   | Yes                         | n/a                 | Yes                  | Yes                 |
...
- You are building a platform where the **V8 trusted tier** runs user tooling and the **QuickJS
  untrusted tier** runs user-submitted or AI-generated code separately.
