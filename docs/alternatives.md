...
Embedding requires cloning the repo. See [getting-started.md](getting-started.md).

## Comparison table

| | **browser-containers** | **WebContainers** | **Nodebox** | **quickjs-emscripten** |
|---|---|---|---|---|
| **License** | Apache 2.0 | Proprietary | MIT | MIT |
| **npm published** | No (workspace-only) | Yes | Yes | Yes |
| **Boot time** | ~100 ms (QuickJS) / ~500 ms (V8 worker) | 2–5 s | ~100 ms | ~50 ms |
| **Bundle size** | TBD | Multi-MB | ~600 KB gzipped | Small |
| **Node.js compat** | Partial (shims) | Full (via WASM) | 40+ polyfills | Limited |
| **Native packages (NAPI)** | No | Yes | No | No |
| **VFS + persistence** | Yes (memfs + OPFS) | Yes | Yes | No |
| **Preview / live server** | Yes (SW + iframe) | Yes | Yes | No |
| **AI agent sandbox** | Yes (QuickJS, C-level caps) | No | No | Yes |
| **High-level `boot()` API** | No | Yes | Yes | No |
| **Dual execution tiers** | Yes (V8 trusted + QuickJS untrusted) | No | No | No |
...
time, and an MIT-licensed package on npm. Sandpack 2.0 uses Nodebox for interactive
code examples. Good choice for documentation sites and tutorials.
...
- **No `boot()` entry point** — must wire VfsBus → SWSandbox → services manually
...
- **Webpack / Next.js** — explicitly out of scope (see [ADR-0003](adr/0003-no-webpack-nextjs.md))
- **ServiceWorker required for preview** — HTTPS or localhost only

See [docs/prd.md](prd.md) for the full scope and non-goals.
